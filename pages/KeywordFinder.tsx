...
export const KeywordFinder: React.FC = () => {
  ...
  // API Key State for manual entry if env is missing
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!hasEnvKey && !userKey) {
        throw new Error("Please enter a Groq API Key to proceed.");
      }

      const data = await findKeywords(topic, keyToUse);
      ...
      setError(error.message || "Failed to analyze keywords.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-3xl mx-auto space-y-4">
        {!hasEnvKey && (
          <div className="bg-amber-900/20 border border-amber-500/30 p-4 rounded-xl animate-fade-in">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-bold text-amber-200">Groq API Key Missing</span>
            </div>
            <Input 
              type="password"
            ...
            />
            <p className="text-[10px] text-slate-500 mt-1 ml-1">Get a free key at console.groq.com. Add VITE_GROQ_API_KEY to .env to hide this.</p>
          </div>
        )}
        ...
        <div className="flex justify-center mt-4 px-4">
          <div className="flex items-center gap-2 text-sm text-slate-400">
             ...
             <span>Professional Mode (Llama 3.3)</span>
          </div>
        </div>
